                                ...
                                <tbody>
                                    ...
                                    @foreach($sections as $section)
                                        <tr>
                                            ...
                                            <td class="hidden-sm hidden-xs hidden-md">{{$section->created_at}}</td>
                                            <td>
                                               {{$section->updated_at}}
                                            </td>
                                            ...
                                            <td>
                                            <a href="editsection/{{$section->id}}" class="btn btn-info" >Edit</a>
                                            </td>
                                            <td>
                                             {!! Form::open(array('url' => 'showsection/' . $section->id, 'class' => '', 'data-toggle' => 'tooltip', 'title' => 'Delete')) !!}
                                                    {!! Form::hidden('_method', 'DELETE') !!}
                                                    {!! Form::button('<i class="fa fa-trash-o fa-fw" aria-hidden="true"></i> <span class="hidden-xs hidden-sm">Delete</span><span class="hidden-xs hidden-sm hidden-md"> Section</span>', array('class' => 'btn btn-danger btn-sm','type' => 'button', 'style' =>'width: 100%;' ,'data-toggle' => 'modal', 'data-target' => '#confirmDelete', 'data-title' => 'Delete Section', 'data-message' => 'Are you sure you want to delete this Section ?')) !!}
                                                {!! Form::close() !!}
                                            </td>
