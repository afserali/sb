                                ...
                                <tbody>
                                    ...
                                    @foreach($regions as $region)
                                        <tr>
                                            ...
                                            <td class="hidden-sm hidden-xs hidden-md">{{$region->created_at}}</td>
                                            <td>
                                               {{$region->updated_at}}
                                            </td>
                                            <td>
                                            <a href="editregion/{{$region->id}}" class="btn btn-info" >Edit</a>
                                            </td>
                                            <td>
                                            {!! Form::open(array('url' => 'showregion/' . $region->id, 'class' => '', 'data-toggle' => 'tooltip', 'title' => 'Delete')) !!}
                                                    {!! Form::hidden('_method', 'DELETE') !!}
                                                    {!! Form::button('<i class="fa fa-trash-o fa-fw" aria-hidden="true"></i> <span class="hidden-xs hidden-sm">Delete</span><span class="hidden-xs hidden-sm hidden-md"> Section</span>', array('class' => 'btn btn-danger btn-sm','type' => 'button', 'style' =>'width: 100%;' ,'data-toggle' => 'modal', 'data-target' => '#confirmDelete', 'data-title' => 'Delete Region', 'data-message' => 'Are you sure you want to delete this Region ?')) !!}
                                                {!! Form::close() !!}
                                            </td>
